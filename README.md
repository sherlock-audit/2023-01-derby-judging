# Issue H-1: pushVaultAmounts can be called multiple times if in the right state 
...
# Issue H-2: Vault executes swaps without slippage protection 
...
# Issue H-3: CompoundProvider's balanceUnderlying and calcShares outputs are scaled incorrectly 
...
# Issue H-4: Anyone can execute certain functions that use cross chain messages and potentially cancel them with potential loss of funds. 

Source: https://github.com/sherlock-audit/2023-01-derby-judging/issues/390 

## Found by 
c7e7eff, Jeiwan, rvierdiiev
...
# Issue M-1: ERC20 return values not checked 

Source: https://github.com/sherlock-audit/2023-01-derby-judging/issues/10 

## Found by 
gmx, Bnke0x0, PRAISE, tsvetanovv, Met, 8olidity, Saeedalipoor01988, Avci, hyh, oot2k, R-Nemes, SunSec
...
This should be low/informational. None of the supported tokens can be exploited with the current implementation:
_ERC20: USDC, DAI, USDT, own DerbyToken and own VaultToken_

The actually provided code snippet works with USDT because it does not check the return value and this never returns one. I understand that using the `SafeTransfer` library is a best practice but in this situation, it should act as a warning, not a bug.
...
Considering this issue as informational in this case
...
# Issue M-2: maxTrainingDeposit can be bypassed 
...
# Issue M-3: Did not Approve to zero first 
...
# Issue M-4: Unprotected slippage tolerance can lead to user/protocol loss of funds 
...
# Issue M-5: Incorrect chainId comparison in xProvider 
...
# Issue M-6: Blacklisting a protocol leads to lower allocations 
...
# Issue M-7: Vault.rebalance calculates rewards per locked token incorrectly 
...
# Issue M-8: MainVault.rebalanceXChain doesn't check that savedTotalUnderlying >= reservedFunds 
...
# Issue M-9: Game doesn't accrued rewards for previous rebalance period in case if rebalanceBasket is called in next period 

Source: https://github.com/sherlock-audit/2023-01-derby-judging/issues/119 

## Found by 
tallo, XKET, rvierdiiev, bin2chen, chainNue
...
# Issue M-10: Rewards per locked token are calculated incorrectly 
...
# Issue M-11: Vault.claim should be called before `pushTotalUnderlyingToController` 
...
# Issue M-12: exchangeRate is not up to date in case if vault that is off is changed to on 
...
# Issue M-13: Vault.blacklistProtocol can revert in emergency 
...
# Issue M-14: There is no price conversion between vault token and provider underlying token amounts in withdrawFromProtocol 
...
# Issue M-15: The protocol could not handle multiple vaults correctly 
...
# Issue M-16: User should not receive rewards for the rebalance period, when protocol was blacklisted, because of unpredicted behaviour of protocol price 

Source: https://github.com/sherlock-audit/2023-01-derby-judging/issues/145 

## Found by 
Bauer, rvierdiiev, Ch\_301, evan, immeas
...
# Issue M-17: minimumPull Vault parameter cannot be adjusted 
...
# Issue M-18: Funds can be frozen on protocol blacklisting 
...
# Issue M-19: Manipulate Allocations from game using flashloans 
...
# Issue M-20: Malicious users could set allocations to a blacklist Protocol and break the rebalancing logic 
...
# Issue M-21: IdleProvider overstates balanceUnderlying() and understates calcShares() 
...
# Issue M-22: Vault can lose rewards due to lack of precision 
...
# Issue M-23: inflate initial share price by initial depositor 
...
# Issue M-25: Wrong calculation of `balanceBefore` and `balanceAfter` in deposit method 
...
# Issue M-26: Vault could `rebalance()` before funds arrive from xChainController 
...
# Issue M-27: vault stakers and game players share the same reward pool 
...
# Issue M-28: `XChainController::sendFundsToVault` can be griefed and leave `XChainController` in a bad state 
...
# Issue M-29: `Vault::pullFunds` doesn't pull funds from underlying providers correctly 
...
# Issue M-30: Should not blacklist protocol when totalUnderlying is not withdrawn. 
...
# Issue M-31: Protocol is will not work on most of the supported blockchains due to hardcoded WETH contract address. 
...
# Issue M-32: Rebalancing can be indefinitely blocked due to ever-increasing `totalWithdrawalRequests`, causing locking of funds in vaults 

Source: https://github.com/sherlock-audit/2023-01-derby-judging/issues/311 

## Found by 
SPYBOY, evan, Jeiwan, nobody2018
...
# Issue M-33: Gamer rewards are reduced due to fund pulls before accrual of rewards 
...
# Issue M-34: Gamers will not receive rewards for allocating to Aave and Beta vaults, while the vaults do generate yield 
...
# Issue M-35: Delayed Compound and Beta interest accrual reduces gamer rewards and affects funds distribution to vaults 
...
# Issue M-36: Deposited funds are locked in inactive vaults 
...
# Issue M-37: The guardian may not be able to blacklist a protocol 
...
# Issue M-38: Missing transaction expiration check result in reward tokens selling at a lower price 
...
# Issue M-39: `XProvider` forces increased relayer fees when transferring tokens cross-chain 
...
# Issue M-40: Rebalancing can be blocked when pulling funds from a TrueFi or a Idle vault 
...
# Issue M-41: Wrong type casting leads to unsigned integer underflow exception when current price is < last price 
...
# Issue M-42: YearnProvider freezes yearn tokens on partial withdrawal 
...
# Issue M-43: Vault's savedTotalUnderlying tracks withdrawn funds incorrectly 
...
# Issue M-44: Current period profit can be extracted from the Vault by front running state change before exchange rate recalculation 
...
# Issue M-45: withdrawal request override 
...
# Issue M-46: Native funds sent with pushVaultAmounts and sendFundsToVault can be lost 
...
# Issue M-47: Rebalancing breaks and can corrupt the accounting if amountToProtocol or amountToChain turn negative 
